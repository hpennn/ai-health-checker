...
from config import PROJECTS, RuntimeConfig, get_project_by_name, CHECKER_IDENTITIES
...
async def lifespan(app: FastAPI):
    ...
    # 启动独立浏览器巡检循环（按需启动-检查-销毁模式，避免 Chromium 常驻内存崩溃）
    global _browser_inspection_task
    bc = get_browser_checker()

    async def _browser_result_callback(result):
        """浏览器巡检结果回调 - 保存并广播"""
        await CheckerManager.save_browser_result(result)

    _browser_inspection_task = asyncio.create_task(
        bc.run_browser_inspection_loop(
            projects=PROJECTS,
            result_callback=_browser_result_callback,
            per_project_sleep_range=(5, 10),
            full_round_sleep=60,
    ...
    if not config.inspection_enabled or not config.is_within_time_range():
        bc.pause_inspection()
        logger.info("[Lifespan] 浏览器巡检初始状态：暂停（总开关关闭或不在时间段内）")
    else:
        logger.info("[Lifespan] 浏览器巡检初始状态：运行中")
    ...
    # 停止独立浏览器巡检循环
    if _browser_inspection_task:
        bc = get_browser_checker()
        bc.stop_inspection()
        _browser_inspection_task.cancel()
...
async def control_inspection(req: ControlRequest):
    ...
    if req.action == "start":
        ...
        if config.is_within_time_range():
            CheckerManager.resume_all()
            AsyncCheckerManager.resume_all()
            get_browser_checker().resume_inspection()
        ...
        CheckerManager.pause_all()
        AsyncCheckerManager.pause_all()
        get_browser_checker().pause_inspection()
